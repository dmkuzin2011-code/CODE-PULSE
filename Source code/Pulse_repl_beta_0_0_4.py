...
from config import start_config_checkout,save_config,config_cmd
...
plugin_loader = PluginLoader(console, CONFIG, HISTORY_FILE)
folder_select=""
# ---------------------------------------------------------------------------
# Autocompletion
# ---------------------------------------------------------------------------
plugin_completions = plugin_loader.completions()
...
def _walk_stats(path: str) -> tuple[int, int, dict[str, int]]:
    ...
    for root, dirs, files in os.walk(path, onerror=lambda e: console.print(f"[yellow]No access: {e.filename}[/yellow]")):
...
def resolve_path(raw: str) -> str:
    ...
    base = folder_select if folder_select else "."
    if folder_select and not os.path.isdir(folder_select):
        console.print(f"[{CONFIG["based_color"]}]Active dir '{folder_select}' no longer exists, falling back to current[/]")
        base = "."
...
def cmd_scan(args: list[str]) -> None:
    ...
    if args[0].startswith("path="):
        ...
        raw = args[0][len("path="):]
        path=resolve_path(raw) if raw else (folder_select or None)
        if not path:
            console.print(f"[red]No path given and no active cd[/]")
            return
...
def cmd_diagram(args: list[str]) -> None:
    ...
    layer = resolve_path(args[0][len("layer="):])
    global folder_select
...
def cmd_support() -> None:
    ...
    console.print("  config theme=<>            — you can change your colortheme.Using hexadecimal shades like #FF00FF.")
    ...
    console.print("  cd                         — show currently active directory.")
    console.print("  cd <dir>                   — set active directory.")
    console.print("  cd main                    — reset active directory back to none.")
    console.print("[dim]Note: scan path=<dir> / diagram layer=<dir> always take priority over cd if given explicitly.[/dim]")
...
def cd(args: list[str]) -> None:   # ← list[str], не str
    global folder_select
    if not args:
        console.print(f"[{CONFIG['based_color']}]Current dir: {folder_select or 'not set'}[/]")
        return
    if args[0] == "main":
        folder_select = ""
        console.print(f"[{CONFIG['based_color']}]Reset to default dir.[/]")
        return
    if os.path.isdir(args[0]):
        folder_select = args[0]
        console.print(f"[{CONFIG['based_color']}]Active dir set to: {folder_select}[/]")
        return
    console.print("[red]CD Error: directory not found[/]")
# ---------------------------------------------------------------------------
# Dispatcher and main
# ---------------------------------------------------------------------------
def dispatch(text: str) -> bool:
    ...
    if cmd in ("exit", "quit"):
        ...
        console.print("[yellow]Beta 0.0.4[/]")
    elif cmd == "save":
        if args and args[0] == "scan_path":
            cmd_export(args[1:])
        else:
            console.print("[yellow]Usage: save scan_path <number> [file=<path>][/]")
    elif cmd == "scan_path":
        cmd_export(args)
    elif cmd == "config":
        config_cmd(args)
        ...
        cd(args)
    elif plugin_loader.has_command(cmd):
        plugin_loader.run(cmd, args)
    elif args and plugin_loader.has_command(f"{cmd} {args[0]}"):
        plugin_loader.run(f"{cmd} {args[0]}", args[1:])
    else:
...
def main() -> None:
    ...
    while True:
        try:
            text = prompt(folder_select+"»", completer=completer_structure)
